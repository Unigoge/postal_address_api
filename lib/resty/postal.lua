...
normalize_options_t libpostal_get_default_options(void);
char **libpostal_expand_address(char *input, normalize_options_t options, size_t *n);
...
void libpostal_address_parser_response_destroy(address_parser_response_t *self);
address_parser_options_t libpostal_get_address_parser_default_options(void);
address_parser_response_t *libpostal_parse_address(char *address, address_parser_options_t options);
...
function postal.expand_address(address, options)
    ...
    local o = lib.libpostal_get_default_options();
...
--    local e = ffi_gc(lib.libpostal_expand_address(c, o, s), lib.libpostal_expansion_array_destroy)
    local e = lib.libpostal_expand_address(c, o, s) 
    local i = -1
    return function()
        i = i + 1
        if i < s[0] then return ffi_string(e[i]) 
        else
            lib.libpostal_expansion_array_destroy( e, s[0] );
        end
    end
end
...
function postal.parse_address(address, language, country)
    ...
    local o = lib.libpostal_get_address_parser_default_options();
    ...
    local c = ffi_new(char_t, #address + 1, address)
    local p = ffi_gc(lib.libpostal_parse_address(c, o), lib.libpostal_address_parser_response_destroy)
